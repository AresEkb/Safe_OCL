...
instantiation simple_type :: order
begin
...
lemma subtype_Boolean_x_intro [intro]:
  ...
  unfolding less_simple_type_def
  using direct_simple_subtype.intros by auto
...
lemma subtype_Integer_x [elim!]:
  ...
  using less_simple_type_def by auto
...
lemma subtype_x_Integer [elim!]:
  ...
  using direct_simple_subtype.simps less_simple_type_def subtype_OclAny_x by auto
...
lemma simple_subtype_acyclic':
  "\<tau> < \<tau> \<Longrightarrow> False"
  for \<tau> :: simple_type
  by (cases \<tau>; auto)

lemma tranclp_less_simple_type:
  "(\<tau>, \<sigma>) \<in> {(\<tau>, \<sigma>). \<tau> \<sqsubset>\<^sub>s \<sigma>}\<^sup>+ \<longleftrightarrow> \<tau> < \<sigma>"
  by (simp add: tranclp_unfold less_simple_type_def)

lemma simple_subtype_acyclic:
  "acyclicP direct_simple_subtype"
  apply (rule acyclicI)
  apply (auto)
  unfolding tranclp_less_simple_type
  by (erule simple_subtype_acyclic')
...
lemma less_le_not_le_simple_type:
  ...
  unfolding less_simple_type_def less_eq_simple_type_def
  apply (rule iffI; auto)
  apply (metis (mono_tags, lifting) less_simple_type_def simple_subtype_acyclic' tranclp_rtranclp_tranclp)
...
instance
  ...
  by (simp add: antisym_simple_type)
...
instance
  apply intro_classes
  apply (simp add: sup_ge1_simple_type)
  apply (simp add: sup_commut_simple_type sup_ge1_simple_type)
...
lemma is_min_simple_type_code [code]:
  "is_min_simple_type \<tau> \<longleftrightarrow>
   ...
   (case \<tau> of ObjectType _ \<Rightarrow> True | _ \<Rightarrow> False)"
...
lemma Boolean_is_min [simp]:
  "is_min_simple_type Boolean"
  by (auto simp add: is_min_simple_type_def)

lemma UnlimitedNatural_is_min [simp]:
  "is_min_simple_type UnlimitedNatural"
  by (auto simp add: is_min_simple_type_def)

lemma String_is_min [simp]:
  "is_min_simple_type String"
  by (auto simp add: is_min_simple_type_def)

lemma ObjectType_is_min [simp]:
  "is_min_simple_type (ObjectType cls)"
  by (auto simp add: is_min_simple_type_def)
...
lemma less_simple_type_code [code_abbrev, simp]:
...
lemma less_eq_simple_type_code [code_abbrev, simp]:
