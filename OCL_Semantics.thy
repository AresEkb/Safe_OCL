theory OCL_Semantics
  imports OCL_Syntax OCL_Values
begin
...
Any property call applied on null results in invalid. Any
operation call applied on null results in invalid, except for the operations specified in 11.3.2 (=, <>, oclAsType, oclIsInState,
oclIsKindOf, oclIsTypeOf, oclIsInvalid, oclIsNew, oclIsUndefined, oclType) and 11.5.4 (and, implies, not, or, xor).
However, by virtue of the implicit conversion to a collection literal, an expression evaluating to null can be used as source
of collection operations (such as ‘isEmpty’). If the source is the null literal, it is implicitly converted to an empty Set by
invoking oclAsSet().
...
*)
*)
inductive big_step :: "expr \<times> 'a val env \<Rightarrow> 'a val \<Rightarrow> bool" (infix "\<Rightarrow>" 55) where
 NullLiteralVal:
...
|UnlimNatLiteralVal:
  "(UnlimitedNaturalLiteral c, e) \<Rightarrow> UnlimNatVal (Just c)"
|StringLiteralVal:
