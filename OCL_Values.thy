...
begin
adhoc_overloading OCL_Values.conj conj
...
adhoc_overloading OCL_Values.conj obool_and
adhoc_overloading OCL_Values.conj ebool_and
...
(* Мы не используем класс errorable, потому что сложение и т.п.
   с бесконечностью равно ошибке. Но мы не можем unat\<^sub>\<bottom> объявить
   экземпляром класс plus *)
...
datatype any = BooleanVal "bool\<^sub>\<bottom>" | UnlimNatVal "eunat" | RealVal "real\<^sub>\<bottom>" | InvalidAny unit

datatype oany = OBooleanVal "bool\<^sub>\<bottom>\<^sub>\<box>" | OUnlimNatVal "eunat\<^sub>\<box>" | ORealVal "real\<^sub>\<bottom>\<^sub>\<box>" | OInvalidAny "unit\<^sub>\<box>"

inductive any_oany :: "any \<Rightarrow> oany \<Rightarrow> bool" where
  "any_oany (BooleanVal x) (OBooleanVal x\<^sub>\<box>)"
...
fun any_to_oany :: "any \<Rightarrow> oany" where
  "any_to_oany (BooleanVal x) = (OBooleanVal x\<^sub>\<box>)"
...
| "cast_any (BooleanVal \<bottom>) (InvalidAny \<bottom>)"
...
| "cast_oany (OBooleanVal \<epsilon>) (OInvalidAny \<epsilon>)"
...
lemma any_to_oany_bij_on_trancl:
  "bij_on_trancl cast_oany any_to_oany"
  apply auto
  apply (simp add: inj_any_to_oany)
  by (simp add: cast_oany_closed_under_range2)

lemma trancl_cast_oany_implies_cast_any:
  "cast_oany\<^sup>+\<^sup>+ (any_to_oany x) (any_to_oany y) \<Longrightarrow> cast_any\<^sup>+\<^sup>+ x y"
  apply (rule reflect_tranclp, auto)
  apply (simp add: cast_oany_implies_cast_any)
  apply (simp add: inj_any_to_oany)
  by (simp add: any_to_oany_bij_on_trancl)

thm preserve_tranclp

lemma trancl_cast_any_implies_cast_oany:
  "cast_any\<^sup>+\<^sup>+ x y \<Longrightarrow> cast_oany\<^sup>+\<^sup>+ (any_to_oany x) (any_to_oany y)"
  by (metis cast_any_implies_cast_oany preserve_tranclp)
...
| RequiredVal any
| OptionalVal oany
| SetVal "'a type" "'a val fset\<^sub>\<bottom>"

definition is_required_set :: "'a val fset \<Rightarrow> bool" where
  "is_required_set xs \<equiv> fBall xs (\<lambda>x. case x of RequiredVal _ \<Rightarrow> True | _ \<Rightarrow> False)"

inductive cast_val_set :: "'a val fset \<Rightarrow> 'a val fset \<Rightarrow> bool" where
  "is_required_set xs \<Longrightarrow>
   cast_val_set xs ys"

code_pred [show_modes] cast_val_set .

fun type_of_any :: "any \<Rightarrow> 'a basic_type" where
  "type_of_any (BooleanVal _) = Boolean"
| "type_of_any (UnlimNatVal _) = UnlimitedNatural"
| "type_of_any (RealVal _) = Real"
| "type_of_any (InvalidAny _) = OclAny"

fun type_of_oany :: "oany \<Rightarrow> 'a basic_type" where
  "type_of_oany (OBooleanVal _) = Boolean"
| "type_of_oany (OUnlimNatVal _) = UnlimitedNatural"
| "type_of_oany (ORealVal _) = Real"
| "type_of_oany (OInvalidAny _) = OclAny"

fun type_of_val :: "'a val \<Rightarrow> 'a type" where
...
fun val_is_ok :: "('a :: order) val \<Rightarrow> bool" where
  "val_is_ok (SetVal \<tau> xs\<^sub>\<bottom>) = fBall xs (\<lambda>x. type_of_val x \<le> \<tau>)"
| "val_is_ok _ = True"

inductive cast_val :: "('a :: order) val \<Rightarrow> 'a val \<Rightarrow> bool" where
  "cast_val (InvalidVal \<bottom>) (VoidVal \<bottom>\<^sub>\<box>)"
| "cast_val (InvalidVal \<bottom>) (RequiredVal (BooleanVal \<bottom>))"
| "cast_val (InvalidVal \<bottom>) (RequiredVal (UnlimNatVal \<bottom>))"
| "cast_val (InvalidVal \<bottom>) (SetVal \<tau> \<bottom>)"
| "cast_val (VoidVal \<bottom>\<^sub>\<box>) (OptionalVal (OBooleanVal \<bottom>\<^sub>\<box>))"
...
(*| "subtype \<tau> \<sigma> \<Longrightarrow>
...
value "cast_val
         (SetVal (UnlimitedNatural[1] :: classes1 type) {|
             RequiredVal (UnlimNatVal (eunat 1)), RequiredVal (UnlimNatVal (eunat 2))|}\<^sub>\<bottom>)
         (SetVal UnlimitedNatural[?] {|
             RequiredVal (UnlimNatVal (eunat 1)), OptionalVal (UnlimNatVal (eunat 3))|}\<^sub>\<bottom>)"
...
inductive_cases cast_oany_OBooleanVal_x[elim!]: "cast_oany (OBooleanVal \<bottom>\<^sub>\<box>) y"
inductive_cases cast_oany_x_OBooleanVal[elim!]: "cast_oany x (OBooleanVal \<bottom>\<^sub>\<box>)"

inductive_cases any_oany_x_OBooleanVal[elim!]: "any_oany x (OBooleanVal \<bottom>\<^sub>\<box>)"

inductive_cases cast_any_BooleanVal_x[elim!]: "cast_any (BooleanVal \<bottom>) y"
...
inductive_cases cast_val_x_InvalidVal2[elim]: "cast_val x (InvalidVal ())"
inductive_cases cast_val_x_VoidVal2[elim]: "cast_val x (VoidVal \<bottom>\<^sub>\<box>)"
...
lemma antisym_cast_val:
  ...
  using cast_val.cases apply blast
  using cast_val.cases subtype_antisym val.distinct(19) by fastforce
...
declare cast_val.intros [intro]


instantiation val :: (order) order
begin

definition "less_val \<equiv> cast_val\<^sup>+\<^sup>+"

definition "less_eq_val \<equiv> cast_val\<^sup>*\<^sup>*"

lemma cast_val_x_VoidVal_intro [intro]:
  "x = InvalidVal \<bottom> \<Longrightarrow> y = \<bottom>\<^sub>\<box> \<Longrightarrow> x < VoidVal y"
  unfolding less_val_def
  using cast_val.intros(1) by blast

lemma cast_val_x_RequiredVal_intro [intro]:
  "x = InvalidVal \<bottom> \<Longrightarrow> y = BooleanVal \<bottom> \<Longrightarrow> x < RequiredVal y"
  "x = InvalidVal \<bottom> \<Longrightarrow> y = UnlimNatVal \<bottom> \<Longrightarrow> x < RequiredVal y"
  "x = RequiredVal z \<Longrightarrow> z < y \<Longrightarrow> x < RequiredVal y"
  unfolding less_val_def
  using cast_val.intros(2) apply auto[1]
  using cast_val.intros(3) apply auto[1]
  by (metis cast_val.intros(7) less_any_def preserve_tranclp)

lemma cast_val_x_OptionalVal_intro [intro]:
  "x = InvalidVal \<bottom> \<Longrightarrow> y = BooleanVal \<bottom> \<Longrightarrow> x < OptionalVal y"
  "x = InvalidVal \<bottom> \<Longrightarrow> y = UnlimNatVal \<bottom> \<Longrightarrow> x < OptionalVal y"
  "x = VoidVal \<bottom>\<^sub>\<box> \<Longrightarrow> y = OBooleanVal \<bottom>\<^sub>\<box> \<Longrightarrow> x < OptionalVal y"
  "x = VoidVal \<bottom>\<^sub>\<box> \<Longrightarrow> y = OUnlimNatVal \<bottom>\<^sub>\<box> \<Longrightarrow> x < OptionalVal y"
  "x = OptionalVal z \<Longrightarrow> z < y \<Longrightarrow> x < OptionalVal y"
  "x = RequiredVal z \<Longrightarrow> z \<le> y \<Longrightarrow> x < OptionalVal y"
  unfolding less_val_def
  apply (metis (mono_tags) cast_val.intros(9) cast_val_x_RequiredVal_intro(1) less_val_def tranclp.trancl_into_trancl)
  apply (metis (mono_tags) any_to_oany.simps(2) cast_val.intros(6) cast_val_x_VoidVal_intro less_val_def tranclp.trancl_into_trancl)
  using cast_val.intros(5) apply auto[1]
  using cast_val.intros(6) apply auto[1]
  apply (metis cast_val.intros(8) less_any_def preserve_tranclp trancl_cast_any_implies_cast_oany)
  by (metis (mono_tags) cast_val.intros(9) cast_val_x_RequiredVal_intro(3) dual_order.order_iff_strict less_val_def tranclp.simps)

lemma cast_val_x_SetVal_intro [intro]:
  "x = InvalidVal \<bottom> \<Longrightarrow> ys' = \<bottom> \<Longrightarrow> x < SetVal \<sigma> ys'"
  "x = SetVal \<tau> xs\<^sub>\<bottom> \<Longrightarrow> \<tau> < \<sigma> \<Longrightarrow>
   rel_fset (\<le>) xs ys \<Longrightarrow> xs \<noteq> ys \<Longrightarrow> x < SetVal \<sigma> ys\<^sub>\<bottom>"
  unfolding less_val_def
  apply (rule tranclp.r_into_trancl)
  using cast_val.intros(4) apply auto[1]
  sorry

lemma cast_val_irrefl:
  "\<tau> < \<tau> \<Longrightarrow> False"
  for \<tau> :: "'a val"
  apply (induct \<tau>, auto)
  apply (metis (mono_tags) cast_val_x_InvalidVal2 less_val_def tranclp.simps)
  sorry
...
instance
  ...
  apply simp
  using OCL_Values.order_trans_val apply auto[1]
  by (simp add: antisym_val)
